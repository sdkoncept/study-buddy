  ...
  return (
    <>
      <nav style={{ marginBottom: "1rem" }}>
        <Link href="/dashboard/subjects" style={{ color: "var(--muted)", fontSize: "0.9rem" }}>
          ← Subjects
        </Link>
      </nav>
          ...
          <li key={t.id}>
            ...
            >
              ...
              <span className="badge badge-muted" style={{ marginTop: "0.5rem" }}>
                {t.estimated_study_time_minutes} min · {t.difficulty_level || "—"}
              </span>
            </Link>
          </li>
        ))}
      ...
      {topicList.length === 0 && (
        <p style={{ color: "var(--muted)" }}>No topics in this subject yet.</p>
      )}
    </>
  );
}
